...
> M4 人工测试期间用户提出的改进需求。**M4 人工测试已于 2026-08-31 通过，本项仍暂缓开发**；此节为记录，后续另行排期实现。

### TODO-A：AI 裁判自然语言技能推断（用户明确需求）

- **现状**：离线兜底裁判靠关键词规则，玩家需在行动里写括号技能名（如 `我检查柜子（侦查）`）才稳定命中；口语化表达（"翻遍整个房间寻找暗门"）命中不了。
- **目标**：玩家纯自然语言行动 → AI 自行推断技能 → 查角色卡取目标值 → 掷骰 → 叙事，**无需括号标注**。
- **实现方案**（曾在会话中实现过半、已回滚，按本方案重做）：
  1. 显式技能名检索：行动文本含中/英文技能名 → 直接采用（兼容括号标注）
  2. 意图短语规则：翻找/寻找/检查→侦查；侧耳/动静/脚步声→倾听；偷偷/溜进→潜行；举起/开枪/射击→手枪；撬开……的锁→开锁（注意"撬开X的锁"≠"撬开锁"）；搭讪→魅惑；套话/打听→套话；威胁/恐吓→威胁 等
  3. 理智特判：看到词（直视/盯着/目睹…）+ 恐怖词（蠕动/血肉/怪物/尸体…）组合，或明确理智词（理智/害怕/san）
  4. 其余 → none（纯叙事推进，不强行加检定）
- **验收**：`我翻遍整个房间寻找暗门`→侦查；`我撬开抽屉的锁`→开锁；`我盯着那团血肉模糊的东西`→理智检定；`我沿着走廊走着`→无需检定。
- **备注**：LLM 在线模式本就把原文交给 LLM 自行判断（已支持自然语言）；本改进主要补齐**离线兜底**体验。

### TODO-B：M4 遗留小项

- 自由掷骰结果不落 `messages` 表（刷新后仅审计可查）→ 是否落库待定。
- 凭证单槽：最近游戏直进若 token 与目标房间不匹配，需回总览重新加入 → M5 按游戏存 token。
