...
## [v0.2.12] - 2026-08-29（未发布）

开局顺序修正。

- `/coc init` 只建房间不立即开场；玩家先 `/coc join` + `/coc build`（或 `/coc use-pregen`）创建角色，全部就绪后 AI 写个性化开场（读角色卡，把名字/职业/特点织进叙事）。

---

## [v0.2.11] - 2026-08-29（未发布）

零字前言铁律强化。

- SKILL.md frontmatter + §1 增加「零字前言铁律」：回复 `/coc` 指令只输出最终结果，绝不输出思考/推理/计划/前言/复述文字。

---

## [v0.2.10] - 2026-08-29（未发布）

彻底关闭 plan-gate。

- `bot-start.ps1` 设 `DSH_LARK_PLAN_GATE=off`（dsh-lark-bot 计划门禁开关），所有 `/coc` 指令直接执行零审批。

---

## [v0.2.9] - 2026-08-29（未发布）

移除真人守密人概念。

- AI 即唯一守密人；玩家可直接 `/coc init` 开局（无需 `--kp` 真人名）。
- `/coc init` 移入通用指令；删除「守密人专用」指令区（scene/npc/reveal/handout/kp-note 改为 AI 内部动作）。
- KP 数据永不输出到任何频道（群聊/私聊都不行，仅供 AI 内部）。

---

## [v0.2.8] - 2026-08-29（未发布）

零 approve + KP 指令群聊锁死。

- `fix-bridge-worktrees.ps1` 反转 junction：worktree 内 `coc-session` 改为真实目录（bot 写入不越界→零 plan-gate），主仓库 `coc-session` 改为 junction 指向它。
- 群聊拒绝 KP 指令；`/coc help` 公开版缓存隐藏守密人专用区。

---

## [v0.2.7] - 2026-08-29（未发布）

工具免审批指引。

- 根因：dsh-lark-bot 默认对每个群的工具权限策略是 `ask`，每次 `/coc` 调脚本都弹「🔐 审批请求」卡片。
- 修复：文档化管理员命令 `/permission allow`（把本群策略改为自动放行）；写入 README 快速开始、DEPLOY 第 5.5 步、SKILL.md §9 边界说明。

---

## [v0.2.6] - 2026-08-29（未发布）

续团/回顾语义自动恢复进度。

- SKILL.md 新增「续团/回顾类问题」硬流程：`/reset` 后新会话没有聊天记忆，但进度在 `coc-session/` 存档里；必须定位房间存档（junction 直读、`git rev-parse` 定位、发现最新房间）并汇总进度，不再回复「没有记忆」。
- frontmatter 描述增强触发：询问「我们上次跑到哪了 / 继续跑团 / 现在什么进度 / 回顾一下」时强制加载本 skill。
